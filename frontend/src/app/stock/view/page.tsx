...
import { DataTable } from "@/app/stock/view/data-table"; // あなたのデータテーブルコンポーネントのパス
      ...
      const handleCheckedChange = async (newCheckedState: boolean) => {
        try {
          ...
          const res = await fetch(
            `http://localhost:8080/stock/view/${item.ID}`,
            {
...
const ItemView = () => {
  ...
  // 成功時のデータテーブル表示
  return (
    <div className="border-2 rounded-[5px] w-[500px] mx-auto mt-[100px] p-3 text-center">
